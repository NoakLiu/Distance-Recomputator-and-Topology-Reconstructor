...
class S2GCLayer(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        for k in range(self.K):
            out = torch.mm(adj, out) + self.alpha[k] * out
...
class S2GC(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, K, nheads):
        super(S2GC, self).__init__()

        self.dropout = dropout

        self.layers = [S2GCLayer(nfeat, nhid, K, dropout) for _ in range(nheads)]
    ...
    def forward(self, x, adj):
        ...
        x = torch.cat([layer(x, adj) for layer in self.layers], dim=1)
        x = F.dropout(x, self.dropout, training=self.training)
...
class S2GCLayer(nn.Module):
    def __init__(self, in_features, out_features, K, dropout):
        ...
        self.weight = nn.Parameter(torch.Tensor(in_features, out_features))
        self.alpha = nn.Parameter(torch.Tensor(K))
        self.batch_norm = nn.BatchNorm1d(out_features)  # Added BatchNorm
    ...
    def forward(self, x, adj):
        ...
        return F.dropout(F.relu(out), self.dropout, training=self.training)  # Added ReLU
"""
