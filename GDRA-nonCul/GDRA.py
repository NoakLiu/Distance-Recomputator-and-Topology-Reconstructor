...
class GraphDistRecompAttentionLayer(nn.Module):
    ...
    def forward(self, h, lamda):
        ...
        cur_mask = torch.where(dischange > lamda, ones_vec, mask) #***替代方案是mask->cur_mask 每次更改某一些节点
        cur_mask = torch.where(dischange < -lamda, zeros_vec, cur_mask)
        attention = torch.where(cur_mask>0, e, zero_vec) #只要mask大于0，就标记了
